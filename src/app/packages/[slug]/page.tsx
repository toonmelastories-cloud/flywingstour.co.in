...
import {
  getPackageBySlug, getRelatedPackages, PackageData,
} from "@/data/packages";
...
function PricingCard({ tier, index, onBook }: {
  tier: PackageData["pricingTiers"][0]; index: number; onBook: () => void;
}) {
...
function BookingSidebar({ pkg, onBook }: { pkg: PackageData; onBook: () => void }) {
  ...
  return (
    <div className="sticky top-28 bg-card border border-border rounded-2xl shadow-card overflow-hidden">
      ...
      <div className="p-6 space-y-4">
        ...
        >
          Book Now — {pkg.startingPrice}/person
        </button>
...
function RelatedCard({ pkg, index }: { pkg: PackageData; index: number }) {
...
export default function PackageDetailPage() {
  ...
  useEffect(() => {
    if (!pkg || pkg.heroImages.length <= 1) return;
    const t = setInterval(() => {
  ...
  useEffect(() => {
    ...
    setMeta("description", pkg.metaDescription);
    setMeta("keywords", pkg.keywords.join(", "));
    setMeta("og:title", pkg.metaTitle, true);
    setMeta("og:description", pkg.metaDescription, true);
    ...
    const breadcrumbSchema = {
      ...
      "@type": "BreadcrumbList",
      itemListElement: [
        { "@type": "ListItem", position: 1, name: "Home", item: "https://flywingstour.in" },
        { "@type": "ListItem", position: 2, name: "Packages", item: "https://flywingstour.in/packages" },
        { "@type": "ListItem", position: 3, name: pkg.shortTitle, item: `https://flywingstour.in/packages/${pkg.slug}` },
      ],
    };

    // TravelAction / TourPackage Schema
    const packageSchema = {
      "@context": "https://schema.org",
      "@type": "TouristTrip",
      name: pkg.title,
      ...
      offers: {
        "@type": "Offer",
        price: pkg.startingPrice.replace(/[₹,]/g, ""),
        priceCurrency: "INR",
    ...
    inject("pkg-faq-schema", faqSchema);
    inject("pkg-breadcrumb-schema", breadcrumbSchema);
    inject("pkg-tour-schema", packageSchema);
  }, [pkg]);
      ...
      <section className="bg-primary border-b border-secondary/20">
        <div className="container-custom py-5">
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            <QuickFact icon={Clock} label="Duration" value={pkg.duration} />
            <QuickFact icon={MapPin} label="Destinations" value={pkg.destinations.join(", ")} />
            <QuickFact icon={Hotel} label="Hotel Category" value={pkg.hotelCategory} />
            <QuickFact icon={Plane} label="Transfers" value={pkg.transferIncluded ? "Included" : "Not Included"} />
            <QuickFact icon={Utensils} label="Meals" value={pkg.mealsIncluded} />
            <QuickFact icon={Users} label="Group Size" value="Private & Group" />
          </div>
        </div>
      </section>
          ...
          <div className="space-y-16 min-w-0">
            ...
            <section id="itinerary">
              <SectionHeader
                ...
                subtitle={`Your complete ${pkg.duration} journey — planned to perfection.`}
              />
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {(day.meals || day.hotel) && (
                          <div className="flex flex-wrap gap-2 mt-1.5">
                            {day.meals && (
                              <span className="inline-flex items-center gap-1 text-[10px] font-body text-muted-foreground bg-muted px-2 py-0.5 rounded-full">
                                <Utensils className="w-2.5 h-2.5" /> {day.meals}
                              </span>
                            )}
                            {day.hotel && (
                              <span className="inline-flex items-center gap-1 text-[10px] font-body text-muted-foreground bg-muted px-2 py-0.5 rounded-full">
                                <Hotel className="w-2.5 h-2.5" /> {day.hotel}
                              </span>
                            )}
                          </div>
                        )}
