...
import { getAllPackages, PackageData } from "@/data/packages";

const FILTERS = ["All", "Dubai", "Thailand", "Bali", "Kashmir", "Maldives", "Singapore"];
const TYPES = ["All Types", "Honeymoon", "Family", "Luxury", "Budget", "Adventure"];
...
function PackageCard({ pkg, index, isInView, onBook }: {
  pkg: PackageData; index: number; isInView: boolean; onBook: () => void;
}) {
      ...
      <div className="p-5">
        ...
        <div className="flex items-end justify-between gap-3">
          <div>
            ...
            <div className="text-primary font-display font-900 text-2xl">{pkg.startingPrice}</div>
...
export default function PackagesPage() {
  ...
  useEffect(() => {
    document.title = "Tour Packages | Dubai, Thailand, Bali & More | Flywings";
    const desc = document.querySelector('meta[name="description"]') || document.createElement("meta");
    desc.setAttribute("name", "description");
    desc.setAttribute("content", "Browse best-value international tour packages from India — Dubai, Thailand, Bali, Kashmir, Maldives, Singapore. Flights, hotels & tours included. Book with Flywings.");
  ...
  const filtered = allPackages.filter((pkg) => {
    ...
    const matchType = activeType === "All Types" ||
      pkg.shortTitle.toLowerCase().includes(activeType.toLowerCase()) ||
      (pkg.badge && pkg.badge.toLowerCase().includes(activeType.toLowerCase()));
    const matchBudget = matchesBudget(pkg.startingPrice, activeBudget);
    return matchSearch && matchDest && matchType && matchBudget;
  });
        ...
        <div className="container-custom" ref={ref}>
          {filtered.length === 0 ? (
            <div className="text-center py-20">
              ...
              <button
                onClick={() => { setSearch(""); setActiveDestination("All"); setActiveBudget("All Budgets"); setActiveType("All Types"); }}
